"""Generate Composio Facebook OAuth link for connecting Facebook pages."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate Composio Facebook OAuth link.")
    parser.add_argument("--org-id", default=None, help="Org/workspace ID to use as user_id.")
    parser.add_argument(
        "--auth-config-id",
        default="ac_E5tGh2QnPSgw",
        help="Facebook auth config ID (default set to existing config).",
    )
