"""Generate Composio Instagram OAuth link for connecting Instagram accounts."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate Composio Instagram OAuth link.")
    parser.add_argument("--org-id", default=None, help="Org/workspace ID to use as user_id.")
    parser.add_argument(
        "--auth-config-id",
        default="ac_w96O-QUUyvzu",
        help="Instagram auth config ID (default set to existing config).",
    )
