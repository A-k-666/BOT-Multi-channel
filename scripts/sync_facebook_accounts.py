"""Sync active Facebook connected accounts into facebook_accounts.json"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Sync active Facebook connected accounts into facebook_accounts.json"
    )
    parser.add_argument(
        "--user-ids",
        help="Comma-separated list of Composio user IDs. "
        "Falls back to env FACEBOOK_COMPOSIO_USER_IDS or COMPOSIO_USER_ID. "
        "If none provided, all Facebook accounts are considered.",
    )
    parser.add_argument(
        "--output",
        default="facebook_accounts.json",
        help="Path of the JSON mapping file (default: facebook_accounts.json).",
    )
    return parser.parse_args()


def resolve_user_ids(args: argparse.Namespace) -> list[str] | None:
    raw = args.user_ids or os.getenv("FACEBOOK_COMPOSIO_USER_IDS")
    if raw:
        return [uid.strip() for uid in raw.split(",") if uid.strip()]
    # If no user_ids specified, return None to fetch ALL Facebook accounts
    return None


def pick_latest_account(accounts):
    """Pick the latest active account for each Facebook page."""
    ...
    for acc in accounts.items:
        ...
        # Extract page_id from state
        page_id = None
        if isinstance(val, dict):
            page_id = val.get("page_id") or val.get("id")
        elif hasattr(val, "page_id"):
            page_id = getattr(val, "page_id", None)
        elif hasattr(val, "id"):
            page_id = getattr(val, "id", None)
        ...
        buckets[page_id].append(acc)

    mapping: dict[str, dict[str, str]] = {}
    for page_id, accs in buckets.items():
...
def main() -> None:
    ...
    # Fetch all Facebook accounts (filter by user_ids only if provided)
    list_kwargs = {"toolkit_slugs": ["FACEBOOK"]}
    if user_ids:
